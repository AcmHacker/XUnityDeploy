	...
	public partial class XCProject : System.IDisposable
	{
		...
		public XCProject( string filePath ) : this()
		{
			...
			if( !string.IsNullOrEmpty( _rootObjectKey ) ) {
                ...
                //tim É¾³ýÎÞÓÃµÄtargets 
                //_project.cleanTargets();
		...
		public PBXDictionary<PBXNativeTarget> nativeTargets {
			get {
				if( _nativeTargets == null ) {
                    ...
                    /*
                    //clear Unity-iPhone Test Target
                    foreach (KeyValuePair<string, PBXNativeTarget> item in _nativeTargets)
                    {
                        var target = item.Value;
                        Debug.Log("target.name " + target.Name);
                        if (target.Name == "Unity-iPhone Tests")
                        {
                            _nativeTargets.Remove(item.Key);
                            break;
                        }
                    }
                    */
		...
		{	
			...
			Debug.Log( "Adding libraries..." + mod.libs.Count);
            ...
            foreach (string key in mod.build_settings.Keys)
            {
                var value = mod.build_settings[key] as string;
                this.overwriteBuildSetting(key, value);
            }
			this.Consolidate();
		}
